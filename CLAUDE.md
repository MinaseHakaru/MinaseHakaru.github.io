...
**「約」は計算結果が不正確だから付けるのではなく、表示のために丸めたときに付ける。** これは数字ツール共通の規則で、複利・現在価値を作るときもそのまま使う。

- **タイル**（`.stat-tile .val`）は値の一覧なので、桁を固定して揃える（実質値の倍率は常に3桁、`1.200倍` / `1.195倍`）。タイルに「約」は出さない。
- **「文章にすると」**は自然言語なので、その値の表示解像度まで丸め、末尾の0を落とし、**元の値とずれた場合だけ**「約」を付ける。単なる文字列の0削除ではなく「人間に読ませる値」として扱う。
- **表示解像度は値ごとに違う。** 倍率は小数第3位、金額は1円。したがって同じ文の中でも判定は独立する（`物価水準は1.2倍になります。…約8,333円分の買い物に相当します` のように、倍率は言い切りで金額だけ「約」になる）。
- **入力か計算結果かで扱いを変えない。** 規則は「文章に表示する値が、その表示解像度への丸めを伴うなら『約』を付ける」であって、値の出どころは関係ない。`1000.5` と入力されたら文章は「いまの約1,001円は」になる。
- **入力欄の値は書き換えない。計算にも入力値そのものを使う。** 丸めるのは文章という表示形式の中だけである（`1000.5` 円のまま計算するので実質値は `834円`。丸めた `1001` で計算すれば `834円` になるが、`1000` 円入力時の `833円` と区別がつかなくなる）。利用者が入れた値を勝手に丸めたのではなく、文章の解像度が1円だというだけ、という状態を保つこと。
- **判定に `===` を使わない。** 浮動小数の誤差（`1.1` の2乗が `1.2100000000000002`、`10000 ÷ 1.25` が `8000.000000000001` になる類）で「約」が付いてしまう。許容差 `EPS = 1e-9` は、どの表示解像度よりも十分小さく、この種の誤差よりは大きい。
